...
2. (optional, permutation) Instead of using a t-test we can also use permutations. A nice visual explanation can be found [here](https://www.jwilber.me/permutationtest/).
```{r}
# to be filled
```
...
6. Calculate the difference in mean expression between samples from male and female patients for each gene (using a for loop or `sapply`) and create a data.frame that contains the p-values, mean expression difference and gene name for each gene.  Save the data.frame using the `saveRDS` function in a file called `DE_results_CLL.RDS`.
```{r}
# to be filled
```
